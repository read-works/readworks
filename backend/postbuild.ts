import { copySync, readFile } from 'fs-extra';
...
const createProdDependencies = async () => {
  const packageDefinition = JSON.parse(await readFile(__dirname + '/package.json', 'utf-8'));
...
const setVersion = async () => {
  const packageDefinition = JSON.parse(await readFile(__dirname + '/dist/package.json', 'utf-8'));
  // ESBUILD STUFF
  ...
  const migrationControllerFileContent = await readFile(migrationControllerFile, 'utf-8');
  await writeFile(
  ...
  const electronFileContent = await readFile(electronFile, 'utf-8');
  await writeFile(
    electronFile,
